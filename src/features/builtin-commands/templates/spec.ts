...
4. **IMPORTANT**: Use \`task\` with \`subagent_type="spec-task-execution"\` for implementation
...
2. **IMPORTANT**: Use \`task\` with \`subagent_type="spec-task-execution"\` to execute the task
...
3. Queue and execute tasks sequentially using \`task\` with \`subagent_type="spec-task-execution"\`
...
**When in \`/spec\` mode, ALWAYS prefer \`task(subagent_type="spec-task-execution")\` over direct implementation.**
