...
Use \`invokeSubAgent(name="explore")\` when you need deep codebase understanding or broad searches.
...
2. **Invoke the requirements-first-workflow subagent** using invokeSubAgent tool
3. **Pass standardized parameters**:
 - name: "requirements-first-workflow"
 - prompt: The original user request with context
 - explanation: Brief explanation of why this subagent is being invoked
...
The orchestrator communicates with subagents using the invokeSubAgent tool:
- **name**: "requirements-first-workflow"
...
## Delegation Protocol
- ALWAYS use invokeSubAgent tool for delegation
...
- You MUST use invokeSubAgent tool with correct subagent ID and context
...
4. **Execute tasks sequentially by delegating to subagents**:
 ...
 b. Call the \`invokeSubAgent\` tool with name="spec-task-execution" - pass the task details in the prompt
